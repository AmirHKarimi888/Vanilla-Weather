...
class HourlyForcast extends AppStructure {
    ...
    generateHTML () {
        ...
        return /*html*/`
        <ul class="border dark:border-zinc-700 bg-gray-100 dark:bg-zinc-800 p-5 rounded-lg grid grid-cols-6 overflow-auto h-full">
           ${
            store.weatherData?.list
            .slice(0, 6)
        .map(item => {
                return `
                <li class="grid justify-center items-center p-2 hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-md cursor-pointer">
